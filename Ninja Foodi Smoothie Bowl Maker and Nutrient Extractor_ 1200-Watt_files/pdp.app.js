  ...
  $ready(function _$ready_pdpRedesign($) {
    ...
    if ($env.enableApolloAnalytics && Kjs.apolloService) {
      if (
        ...
        Kjs.apolloService.productOfferViewedAnalytics();
      }

      Kjs.apolloService.productViewedAnalytics(productV2JsonData);
      Kjs.apolloService.userAnalytics.userDataAnalytics();
      Kjs.apolloService.userAnalytics.userVisitStartedAnalytics();
      Kjs.apolloService.productLocationListingItemClickedAnalytics();
      Kjs.apolloService.productListingItemClickedAnalytics();
      Kjs.apolloService.pageLoadCompleteAnalytics();
      Kjs.apolloService.listenForCTALinkClick();
    }
  });
  ...
  $init(
    function _$init_pdpRedesignBoot($) {
      ...
      if (
        ...
        Kjs.apolloService.pageLoadAnalytics({
          category: (productV2JsonData.breadcrumbs || [{}])[0].name,
          productTitle: productV2JsonData.productTitle,
          pageType: "pdp",
        });
        Kjs.apolloService.locationAnalytics(
          Kjs.cookie.get("K_favstore").split("|")[1] ||
            (productV2JsonData.storeInfo || {}).storeId
        );
